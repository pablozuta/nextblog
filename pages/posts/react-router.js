  ...
  return (
    <>
      ...
      <p>
        ...
        you build your app? What exactly is a router, anyway? If youve ever had
        any of these questions, or youd just like to dig into the fundamental
        pieces of routing, youre in the right place. This document contains
        detailed explanations of all the core concepts behind routing as
        implemented in React Router.
      </p>
      <p>
        Please dont let this document overwhelm you! For everyday use, React
        Router is pretty simple. You dont need to go this deep to use it. React
        Router isnt just about matching a url to a function or component: its
        about building a full user interface that maps to the URL, so it might
        have more concepts in it than youre used to. Well go into detail on
      ...
      <ul>
        <li>URL - The URL in the address bar. A lot of people use the term URL and route interchangeably, but this is not a route in React Router, its just a URL.</li>
        <li>Location - This is a React Router specific object that is based on the built-in browsers window.location object. It represents where the user is at. Its mostly an object representation of the URL but has a bit more to it than that.</li>
        <li>Location State - A value that persists with a location that isnt encoded in the URL. Much like hash or search params (data encoded in the URL), but stored invisibly in the browsers memory.</li>
        <li>History Stack - As the user navigates, the browser keeps track of each location in a stack. If you click and hold the back button in a browser you can see the browsers history stack right there.</li>
...
Segment - The parts of a URL or path pattern between the / characters. For example, /users/123 has two segments.</li>
        <li>Path Pattern - These look like URLs but can have special characters for matching URLs to routes, like dynamic segments (/users/:userId) or star segments (/docs/*). They arent URLs, theyre patterns that React Router will match.</li>
        ...
        <li>Route Element - Or <code>Route</code>  . This elements props are read to create a route by <code>Routes</code> , but otherwise does nothing.</li>
        ...
        <li>Relative links - Links that dont start with / will inherit the closest route in which they are rendered. This makes it easy to link to deeper URLs without having to know and build up the entire path.</li>
        ...
        <li>Index Route - A child route with no path that renders in the parents outlet at the parents URL.</li>
